            ...
            if workflows_file.exists():
                with open(workflows_file, 'r') as f:
                    ...
                    for workflow_data in data[-1000:]:  # Load last 1000
                        workflow = WorkflowMetrics(**workflow_data)
                        # Convert datetime strings back to datetime objects
                        workflow.start_time = datetime.fromisoformat(workflow_data['start_time'])
                        if workflow_data.get('end_time'):
                            workflow.end_time = datetime.fromisoformat(workflow_data['end_time'])
        ...
        try:
            ...
            for workflow in self.recent_workflows:
                ...
                if workflow.end_time:
                    workflow_dict['end_time'] = workflow.end_time.isoformat()
                
                # Convert node datetime objects
                for node_dict in workflow_dict['nodes']:
                    node_dict['start_time'] = datetime.fromisoformat(node_dict['start_time']).isoformat() if isinstance(node_dict['start_time'], str) else node_dict['start_time'].isoformat()
                    if node_dict.get('end_time'):
                        node_dict['end_time'] = datetime.fromisoformat(node_dict['end_time']).isoformat() if isinstance(node_dict['end_time'], str) else node_dict['end_time'].isoformat()
                
                serializable_workflows.append(workflow_dict)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        workflow_id = dashboard.start_workflow("youtube")
        
        # Simulate node executions
        nodes = ["content_fetcher", "summarizer", "embedding", "storage"]
        for node in nodes:
